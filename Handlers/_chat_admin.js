...
const commandList = {
	...
	'help': {
		...
		handler: function (player, args) {
			...
			messages.forEach((message, i) => {
				var HandleQuestUpdate = global.Network.createPacket('HandleQuestUpdate');
	...
	'packetInspector': {
		...
		handler: function (player, args) {
			global.Logging.changeOptions({
				packet: global.Logging.output[args[1]] || global.Logging.output.websocket,
			});
			player.chatBoxMessage(`packet logging: ${global.Logging.options.packet.name}`);
		},
	},
	'levelup': {
	...
	'test': {
		...
		handler: function (player, args) {
			for (let i = 22; i > 0; i--) {
				var unit = global.Barracks['RED'][0].spawnUnit('Basic');
				//unit.moveLane = () => {};
	...
	'wave': {
		...
		arguments: [],
		handler: function (player, args) {
			global.Barracks['BLUE'][1]?.spawnWave();
			global.Barracks['RED'][1]?.spawnWave();
		}
	},

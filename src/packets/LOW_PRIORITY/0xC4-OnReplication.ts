...
export default class OnReplication extends BasePacket {
	...
	Replicant_PLAYER(unit: Player) {//todo: some values may be wrong
	...
	UpdateReplicant(unit: Unit, value: number | undefined, primaryId: number, secondaryId: number) {
	...
	writer(buffer: PacketReaderWriter) {
		...
		for (let i = 0; i < this.count; i++) {
			...
			let unitType = (unit.type || 'Player') as keyof typeof replicantListNames;
	...
	reader(buffer: PacketReaderWriter) {
		...
		for (let i = 0; i < this.count; i++) {
			let unit = this.units[i] = {
				...
				type: 'Player' as keyof typeof replicantListNames,
				//netId >= 0xFF000000 ? 'Turret' :
				//netId >= 0x40000000 ? 'Minion' :
				seconadaryIdArray: [] as number[],
				dataCount: [] as number[],
				replicant: [] as { [x: number]: [string, number] }[],
			};
			...
			for (let primaryId = 0; primaryId < 6; primaryId++) {
				...
				let replicantPrimary = unit.replicant[primaryId] = {} as { [x: number]: [string, number] };

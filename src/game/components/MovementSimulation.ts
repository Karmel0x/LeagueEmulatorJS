...
import { MovableObject, MovableUnit } from '../../gameobjects/GameObjects';
...
export default class MovementSimulation {
	 ...
	 */
	move(unit: MovableObject, diff: number) {
	 ...
	 */
	callbacks(unit: MovableObject, diff: number) {
	...
	async update() {
		for (; ;) {
			...
			let units = UnitList.getUnits() as MovableUnit[];
			units.forEach(unit => {
				if (!unit.moving)
					return;

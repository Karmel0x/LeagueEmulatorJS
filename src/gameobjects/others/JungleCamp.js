...
module.exports = class JungleCamp extends ExtendWTraits(GameObject) {
	...
	notifyCreate() {
		var CreateMinionCamp = Server.network.createPacket('CreateMinionCamp');
		CreateMinionCamp.position = this.position;
		CreateMinionCamp.campIndex = this.num;
		Server.teams.ALL.sendPacket(CreateMinionCamp, loadingStages.LOADING);
	}

	notifyActivate() {
		var ActivateMinionCamp = Server.network.createPacket('ActivateMinionCamp');
		ActivateMinionCamp.position = this.position;
		ActivateMinionCamp.campIndex = this.num;
		Server.teams.ALL.sendPacket(ActivateMinionCamp, loadingStages.LOADING);
	}

	notifyDeactivate() {
		var Neutral_Camp_Empty = Server.network.createPacket('Neutral_Camp_Empty');
		Neutral_Camp_Empty.campIndex = this.num;
		Server.teams.ALL.sendPacket(Neutral_Camp_Empty, loadingStages.LOADING);
	}

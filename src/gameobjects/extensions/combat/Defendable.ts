...
import { AttackableUnit, DefendableUnit } from '../../GameObjects';
...
export default class Defendable {
	...
	constructor(owner: DefendableUnit) {
		this.owner = owner;
	}

	UnitApplyDamage(source: AttackableUnit, damage: { resultType?: number; type?: number; amount?: number; }) {
		const packet1 = new packets.UnitApplyDamage();
		packet1.netId = this.owner.netId;
		packet1.damageResultType = damage.resultType || packets.UnitApplyDamage.DamageResultType.normal;
		packet1.unk1 = 125;
		packet1.damageType = damage.type || packets.UnitApplyDamage.DamageType.mixed;
	...
	damage(source: AttackableUnit, dmg: { ad: number; }) {

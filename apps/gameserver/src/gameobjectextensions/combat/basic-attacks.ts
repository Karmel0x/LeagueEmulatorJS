...
export default class BasicAttacks {
	...
	constructor(owner: AttackableUnit) {
		...
		owner.eventEmitter.on('preCastSpell', (spell, targets, e) => {
			if (!targets)
				return;
			...
			// TODO: check why owner.stats.attackSpeed.total may be NaN
			const attackSpeedTotal = owner.stats.attackSpeed.total || 0.625;
			const attackSpeedMod = 1 + Math.min(attackSpeedTotal, 2.55) - 0.625;
		...
		owner.eventEmitter.on('cancelSpellCast', (spell) => {
			if (!spell)
				return;
